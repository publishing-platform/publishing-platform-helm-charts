...
git config --global user.email "${GIT_NAME}@publishing-platform.co.uk"
git config --global user.name "${GIT_NAME}"

gh auth setup-git
cd /tmp
gh repo clone publishing-platform/publishing-platform-helm-charts -- --depth 1 --branch main

cd "publishing-platform-helm-charts" || exit 1
...
gh api repos/publishing-platform/publishing-platform-helm-charts/merges -f head="${BRANCH}" -f base=main
git push origin --delete "${BRANCH}"
