...
for values_filename in ./charts/app-config/values-*.yaml; do
  env=$(yq -r ".publishingPlatformEnvironment" "$values_filename")

  mkdir -p "output/rendered-charts/${env}"
  mkdir -p "output/values/${env}"

  array_length=$(yq e ".publishingPlatformApplications | length - 1" "$values_filename")
  ...
  yq 'del(.publishingPlatformApplications)' "/tmp/env-values.yaml" > "/tmp/env-values-no-apps.yaml"

  for app_index in $(seq 0 "$array_length"); do
    app_name=$(yq ".publishingPlatformApplications[$app_index].name" "$values_filename")

    chart_output="./output/rendered-charts/${env}/${app_name}"

    chart_path=$(yq ".publishingPlatformApplications[$app_index].chartPath // \"charts/generic-publishing-platform-app\"" "$values_filename");
    ...
    yq ".publishingPlatformApplications[$app_index].helmValues" "/tmp/env-values.yaml" > "$app_values";
